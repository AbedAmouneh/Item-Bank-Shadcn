...
const ViewCard = styled(Paper)(({ theme }) => ({
  backgroundColor: theme.palette.semantic.questionView.background,
  border: `1px solid ${theme.palette.semantic.questionView.border}`,
}));

const QuestionViewShell = ({ question }: QuestionViewShellProps) => {

  return (
    <ViewCard className="p-6 rounded-xl" elevation={0}>
       <Typography className="text-[1.25rem] font-semibold py-4 px-0" sx={theme => ({color: theme.palette.text.primary})}>{question?.questionName}</Typography>
       {question?.type !== 'select_correct_word' && question?.type !== 'highlight_correct_word' && question?.type !== 'drag_drop_text' && question?.type !== 'drag_drop_image' && question && (
         <Typography sx={(theme) => ({color: theme.palette.semantic.questionView.questionTextColor})} className="text-[0.95rem] font-medium mb-2" dangerouslySetInnerHTML={{__html: question.question_text}} />
       )}
       {
